...
from .jobs import run_alerts_job, run_cleanup_old_tenders_job, run_ingest_job
    ...
    async def _run_ingest_job_wrapper(self) -> None:
        ...
        try:
            ...
            if result["status"] == "success":
                ...
                job_logger.error(
                    f"Scheduled ingestion failed: {result.get('error', 'Unknown error')}"
                )
        except Exception as e:
    ...
    async def _run_cleanup_job_wrapper(self) -> None:
        ...
        try:
            ...
            if result["status"] == "success":
                ...
                job_logger.error(
                    f"Scheduled cleanup failed: {result.get('error', 'Unknown error')}"
                )
        except Exception as e:
    ...
    async def _run_alerts_job_wrapper(self) -> None:
        ...
        try:
            ...
            if result["status"] == "success":
                ...
                job_logger.error(
                    f"Scheduled alerts job failed: {result.get('error', 'Unknown error')}"
                )
        except Exception as e:
