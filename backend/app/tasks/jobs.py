...
async def run_ingest_job(ted_limit: int = 200, boamp_limit: int = 200) -> Dict[str, Any]:
...
async def run_boamp_ingest_job(limit: int = 200) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "status": "error",
            "error": str(e),
            "message": "BOAMP ingestion failed"
        }
...
async def cleanup_old_tenders_job(days_old: int = 365) -> Dict[str, Any]:
    ...
    job_logger.info(f"Starting cleanup job (removing tenders older than {days_old} days)")
    ...
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            count_result = await db.execute(
                ...
                .where(Tender.publication_date < cutoff_date)
            )
