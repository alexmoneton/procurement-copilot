...
class OutreachTargetingService:
            ...
            if award.other_bidders:
                for bidder_name in award.other_bidders:
                    if bidder_name.strip():
                        company_counts[bidder_name] = (
                            company_counts.get(bidder_name, 0) + 1
                        )
    ...
    async def get_single_country_bidders_with_cross_border_potential(
        self, db: AsyncSession, cpv_codes: Optional[List[str]] = None, limit: int = 200
    ) -> List[Dict[str, Any]]:
        """
        ...
        if cpv_codes:
            awards_query = awards_query.join(
                Tender, Award.tender_ref == Tender.tender_ref
            )
            cpv_conditions = []
            for cpv in cpv_codes:
                cpv_conditions.append(Tender.cpv_codes.contains([cpv]))
        ...
        for company_name, country_bids in company_country_bids.items():
            ...
            strong_countries = [
                country for country, count in country_bids.items() if count >= 3
            ]
            ...
            for strong_country in strong_countries:
                ...
                has_neighbor_bids = any(
                    country_bids.get(neighbor, 0) > 0 for neighbor in neighbor_countries
                )
        ...
        self.logger.info(
            f"Found {len(cross_border_candidates)} cross-border potential companies"
        )
    ...
    async def get_lapsed_bidders(
        ...
        active_period_query = select(Award).where(
            and_(
                Award.award_date >= twelve_months_ago, Award.award_date < six_months_ago
            )
        )
        ...
        if cpv_codes or country:
            active_period_query = active_period_query.join(
                Tender, Award.tender_ref == Tender.tender_ref
            )
            lapsed_period_query = lapsed_period_query.join(
                Tender, Award.tender_ref == Tender.tender_ref
            )
