...
from ..db.crud import EmailLogCRUD, SavedFilterCRUD
from ..db.models import EmailLog, NotifyFrequency, SavedFilter, Tender, User
...
class AlertService:
    ...
    async def _process_filter(
        self, db: AsyncSession, saved_filter: SavedFilter
    ) -> Dict[str, Any]:
        """Process a single saved filter."""
        self.logger.info(
            f"Processing filter: {saved_filter.name} (ID: {saved_filter.id})"
        )
        ...
        if email_result["status"] == "sent":
            ...
            await self._log_email(
                db, saved_filter, user, email_result, matching_tenders
            )
    ...
    async def _find_matching_tenders(
        self, db: AsyncSession, saved_filter: SavedFilter
    ) -> List[Dict[str, Any]]:
            ...
            for country in saved_filter.countries:
                if country.strip():
                    country_conditions.append(
                        Tender.buyer_country == country.strip().upper()
                    )
        ...
        self.logger.info(
            f"Found {len(tender_dicts)} matching tenders for filter '{saved_filter.name}'"
        )
        return tender_dicts
    ...
    async def _log_email(
        ...
        self.logger.info(
            f"Logged email for filter '{saved_filter.name}' to {user.email}"
        )

    async def send_alerts_for_filter(
        self, db: AsyncSession, filter_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not saved_filter:
                return {"status": "error", "error": f"Filter not found: {filter_id}"}

            return await self._process_filter(db, saved_filter)

        except ValueError:
            return {"status": "error", "error": f"Invalid filter ID: {filter_id}"}
        except Exception as e:
