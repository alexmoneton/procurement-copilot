...
async def get_tenders_seo(
    ...
    try:
        ...
        for tender in tenders:
            ...
            category = _map_cpv_to_category(tender.cpv_codes) if tender.cpv_codes else "General"
            budget_band = _map_value_to_budget_band(tender.value_amount) if tender.value_amount else "Unknown"
            year = tender.publication_date.year if tender.publication_date else datetime.now().year
            
            seo_tenders.append(TenderSEO(
                id=str(tender.id),
                title=tender.title,
                ...
                deadline=tender.deadline_date.isoformat() if tender.deadline_date else "",
                url=tender.url,
                value_amount=float(tender.value_amount) if tender.value_amount else None,
...
async def get_or_create_page_intro(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error generating page intro: {str(e)}")
...
async def get_sitemap_data(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error generating sitemap data: {str(e)}")
...
async def get_country_stats(country: str, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        if not tenders:
            raise HTTPException(status_code=404, detail="No tenders found for this country")
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching country stats: {str(e)}")
...
async def _generate_page_intro(country: str, category: str, year: int, budget: str) -> str:
