...
if stripe and settings:
    ...
    if stripe_secret_key:
        ...
        print(
            f"DEBUG: Stripe secret key starts with: {stripe.api_key[:10] if stripe.api_key else 'None'}"
        )
    else:
        print("DEBUG: No Stripe secret key found in settings")
        logger.warning(
            "Stripe secret key not configured - billing endpoints will be disabled"
        )
...
async def test_checkout(
    ...
    try:
        ...
        try:
            ...
            return {"status": "error", "stripe_configured": True, "error": str(e)}
...
async def handle_subscription_updated(subscription):
    ...
    logger.info(
        f"Subscription updated: {subscription['id']} for customer {customer_id}"
    )
...
async def handle_subscription_deleted(subscription):
    ...
    customer_id = subscription.get("customer")
    logger.info(
        f"Subscription canceled: {subscription['id']} for customer {customer_id}"
    )
    # In a real app, you'd update the subscription status in your database
