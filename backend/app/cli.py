...
from .db.crud import CompanyCRUD, SavedFilterCRUD, UserCRUD
from .db.models import NotifyFrequency
from .db.schemas import CompanyCreate, SavedFilterCreate, UserCreate
...
@cli.command()
@click.option(
    "--source", type=click.Choice(["TED", "BOAMP_FR"]), help="Source to ingest from"
)
    ...
    async def _create_filter():
        ...
        try:
            ...
            async with AsyncSessionLocal() as db:
                ...
                country_list = (
                    [c.strip().upper() for c in countries.split(",")]
                    if countries
                    else []
                )
                ...
                click.echo(
                    f"Filter created successfully: {saved_filter.name} (ID: {saved_filter.id})"
                )
        finally:
            await close_db()
            ...
            async with AsyncSessionLocal() as db:
                if email:
                    ...
                    for saved_filter, user_email in filter_data:
                        ...
                        click.echo(
                            f"    Keywords: {', '.join(saved_filter.keywords) if saved_filter.keywords else 'None'}"
                        )
                        click.echo(
                            f"    CPV: {', '.join(saved_filter.cpv_codes) if saved_filter.cpv_codes else 'None'}"
                        )
                        click.echo(
                            f"    Countries: {', '.join(saved_filter.countries) if saved_filter.countries else 'None'}"
                        )
                        click.echo(
                            f"    Frequency: {saved_filter.notify_frequency.value}"
                        )
...
@click.option(
    "--strategy",
    required=True,
    type=click.Choice(["lost_bidders", "cross_border", "lapsed"]),
    help="Targeting strategy",
)
...
@click.option(
    ...
    required=True,
    type=click.Choice(
        ["missed_opportunities", "cross_border_expansion", "reactivation"]
    ),
    help="Campaign type",
)
@click.option(
        ...
        try:
            ...
            async with AsyncSessionLocal() as db:
                ...
                leads = await outreach_engine.build_lead_list(
                    ...
                    cpv_codes,
                    country,
                    limit * 2,  # Get more leads than we'll send
                )
                ...
                # Send campaign
                click.echo(
                    f"Sending {campaign} campaign to {min(len(leads), limit)} leads..."
                )
            ...
            async with AsyncSessionLocal() as db:
                ...
                if results["error_details"]:
                    ...
                    if len(results["error_details"]) > 10:
                        click.echo(
                            f"  ... and {len(results['error_details']) - 10} more errors"
                        )
...
@click.option(
    "--limit", type=int, default=50, help="Maximum number of companies to show"
)
def list_companies(country: str, limit: int):
    ...
    async def _list_companies():
        ...
        try:
            ...
            async with AsyncSessionLocal() as db:
                if country:
                    companies = await CompanyCRUD.get_companies_by_country(
                        db, country, limit
                    )
                else:
