...
from app.db.crud import TenderCRUD, UserCRUD, SavedFilterCRUD
from app.db.schemas import TenderCreate, TenderUpdate, UserCreate, SavedFilterCreate, SavedFilterUpdate
...
class TestTenderCRUD:
    ...
    async def test_get_tender_by_ref(self, test_db, test_tender_data):
        ...
        retrieved_tender = await TenderCRUD.get_by_ref(test_db, test_tender_data["tender_ref"])
    ...
    async def test_update_tender(self, test_db, test_tender_data):
        ...
        updated_tender = await TenderCRUD.update(test_db, created_tender.id, update_data)
    ...
    async def test_search_tenders_with_date_filters(self, test_db, sample_tenders):
        ...
        tenders, total = await TenderCRUD.search(test_db, from_date=from_date, to_date=to_date)
        
        assert len(tenders) >= 0
        assert total >= 0
        assert all(from_date <= tender.publication_date <= to_date for tender in tenders)
    ...
    async def test_search_tenders_with_value_filters(self, test_db, sample_tenders):
        ...
        tenders, total = await TenderCRUD.search(test_db, min_value=min_value, max_value=max_value)
...
class TestSavedFilterCRUD:
    ...
    @pytest.mark.asyncio
    async def test_create_saved_filter(self, test_db, sample_user, test_saved_filter_data):
        """Test creating a saved filter."""
        filter_create = SavedFilterCreate(**test_saved_filter_data)
        created_filter = await SavedFilterCRUD.create(test_db, filter_create, sample_user.id)
        ...
        assert created_filter.notify_frequency == test_saved_filter_data["notify_frequency"]
    
    @pytest.mark.asyncio
    async def test_get_saved_filter_by_id(self, test_db, sample_saved_filter):
        """Test getting a saved filter by ID."""
        retrieved_filter = await SavedFilterCRUD.get_by_id(test_db, sample_saved_filter.id)
    ...
    async def test_update_saved_filter(self, test_db, sample_saved_filter):
        ...
        update_data = SavedFilterUpdate(
            ...
            min_value=Decimal("75000.00")
        )
        
        updated_filter = await SavedFilterCRUD.update(test_db, sample_saved_filter.id, update_data)
