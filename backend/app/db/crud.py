...
from .models import (Award, Company, EmailLog, SavedFilter, Tender,
                     TenderSource, User, UserProfile)
from .schemas import (AwardCreate, CompanyCreate, CompanyUpdate,
                      EmailLogCreate, SavedFilterCreate, SavedFilterUpdate,
                      TenderCreate, TenderUpdate, UserCreate,
...
class TenderCRUD:
    ...
    @staticmethod
    async def update(
        db: AsyncSession, tender_id: uuid.UUID, tender_update: TenderUpdate
    ) -> Optional[Tender]:
        """Update tender."""
        result = await db.execute(select(Tender).where(Tender.id == tender_id))
...
class SavedFilterCRUD:
    ...
    @staticmethod
    async def create(
        db: AsyncSession, filter_data: SavedFilterCreate, user_id: uuid.UUID
    ) -> SavedFilter:
    ...
    @staticmethod
    async def get_by_id(
        db: AsyncSession, filter_id: uuid.UUID
    ...
    async def get_daily_filters(db: AsyncSession) -> list[SavedFilter]:
        ...
        result = await db.execute(
            select(SavedFilter).where(
                SavedFilter.notify_frequency == NotifyFrequency.DAILY
            )
        )
        return list(result.scalars().all())
    ...
    @staticmethod
    async def update(
        db: AsyncSession, filter_id: uuid.UUID, filter_update: SavedFilterUpdate
    ) -> Optional[SavedFilter]:
        """Update saved filter."""
        result = await db.execute(
            select(SavedFilter).where(SavedFilter.id == filter_id)
...
class EmailLogCRUD:
    ...
    @staticmethod
    async def get_by_id(
        db: AsyncSession, email_log_id: uuid.UUID
    ) -> Optional[EmailLog]:
        """Get email log by ID."""
    ...
    async def get_by_filter(
        db: AsyncSession, filter_id: uuid.UUID, limit: int = 50
    ) -> list[EmailLog]:
...
class AwardCRUD:
    ...
    async def get_recent_awards(db: AsyncSession, limit: int = 100) -> list[Award]:
        ...
        result = await db.execute(
            select(Award).order_by(desc(Award.award_date)).limit(limit)
        )
        return list(result.scalars().all())
    ...
    async def get_awards_by_date_range(
        ...
        result = await db.execute(
            select(Award)
            .where(and_(Award.award_date >= start_date, Award.award_date <= end_date))
...
class CompanyCRUD:
    ...
    async def get_by_name_and_country(
        db: AsyncSession, name: str, country: str
    ) -> Optional[Company]:
    ...
    async def get_companies_by_country(
        db: AsyncSession, country: str, limit: int = 100
    ) -> list[Company]:
    ...
    @staticmethod
    async def update(
        db: AsyncSession, company_id: uuid.UUID, company_update: CompanyUpdate
    ) -> Optional[Company]:
        """Update a company."""
        result = await db.execute(select(Company).where(Company.id == company_id))
    ...
    async def upsert_by_name_and_country(
        db: AsyncSession, company: CompanyCreate
    ) -> Company:
    ...
    @staticmethod
    async def update_last_contacted(db: AsyncSession, company_id: uuid.UUID) -> bool:
...
class UserProfileCRUD:
    ...
    @staticmethod
    async def create(
        db: AsyncSession, user_id: uuid.UUID, profile: UserProfileCreate
    ) -> UserProfile:
