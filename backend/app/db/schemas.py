...
from .models import NotifyFrequency, TenderSource
...
class Tender(TenderBase):
    ...
    competition_level: Optional[str] = Field(
        None, description="Expected competition level"
    )
    deadline_urgency: Optional[str] = Field(
        None, description="Deadline strategy advice"
    )
...
class UserProfileBase(BaseModel):
    ...
    target_value_range: Optional[list[int]] = Field(
        None, description="Target value range [min, max]"
    )
    preferred_countries: Optional[list[str]] = Field(
        None, description="Preferred countries"
    )
...
class SavedFilterBase(BaseModel):
    ...
    notify_frequency: NotifyFrequency = Field(
        NotifyFrequency.DAILY, description="Notification frequency"
    )
