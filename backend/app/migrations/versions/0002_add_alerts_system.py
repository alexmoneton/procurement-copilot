...
def upgrade() -> None:
    ...
    op.add_column('saved_filters', sa.Column('name', sa.String(length=255), nullable=False, server_default='Unnamed Filter'))
    
    # Add last_notified_at column to saved_filters
    op.add_column('saved_filters', sa.Column('last_notified_at', sa.DateTime(timezone=True), nullable=True))
    ...
    op.create_index(op.f('ix_email_logs_saved_filter_id'), 'email_logs', ['saved_filter_id'], unique=False)
