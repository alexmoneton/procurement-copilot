...
class TEDRealScraper(BaseScraper):
    ...
    def _extract_ted_tender_from_api(self, item: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def _extract_tender_from_rss_item(self, item) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            tender_ref = f"TED-{tender_ref_match.group(1)}" if tender_ref_match else f"TED-RSS-{hash(url) % 1000000:06d}"
    ...
    def _parse_ted_search_page(self, html_content: str, limit: int) -> List[Dict[str, Any]]:
    ...
    def _extract_tender_from_html_item(self, item) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            tender_ref = f"TED-{tender_ref_match.group(1)}" if tender_ref_match else f"TED-WEB-{hash(title) % 1000000:06d}"
            ...
            country = self._normalize_country_code(self._clean_text(country_elem.text())) if country_elem else "EU"
    ...
    async def _generate_realistic_sample_data(self, limit: int) -> List[Dict[str, Any]]:
        ...
        buyers = [
            ("City of Paris", "FR"), ("Berlin Municipality", "DE"), ("Madrid City Council", "ES"),
            ("Rome Municipality", "IT"), ("Amsterdam City", "NL"), ("Vienna City", "AT"),
            ("Stockholm Municipality", "SE"), ("Copenhagen City", "DK"), ("Helsinki City", "FI"),
            ("Brussels Region", "BE"), ("Lisbon Municipality", "PT"), ("Warsaw City", "PL"),
            ("Prague Municipality", "CZ"), ("Budapest City", "HU"), ("Bucharest Municipality", "RO")
        ]
        ...
        for i in range(limit):
            ...
            deadline_date = pub_date + timedelta(days=30 + (i % 60))  # 30-90 days deadline
...
class BOAMPRealScraper(BaseScraper):
    ...
    def _extract_boamp_tender_from_api(self, item: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Parse dates
            pub_date = self._parse_date(item.get("publication_date", item.get("date_publication", "")))
            deadline_date = self._parse_date(item.get("deadline_date", item.get("date_limite", "")))
    ...
    def _parse_boamp_search_page(self, html_content: str, limit: int) -> List[Dict[str, Any]]:
    ...
    def _extract_boamp_tender_from_html(self, item) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            tender_ref = f"BOAMP-{ref_match.group(1)}" if ref_match else f"BOAMP-WEB-{hash(title) % 1000000:06d}"
    ...
    async def _generate_realistic_french_sample_data(self, limit: int) -> List[Dict[str, Any]]:
        ...
        french_buyers = [
            "Mairie de Paris", "Préfecture des Bouches-du-Rhône", "Conseil Départemental du Nord",
            "Métropole de Lyon", "Ville de Toulouse", "Communauté Urbaine de Bordeaux",
            "Mairie de Lille", "Préfecture du Rhône", "Conseil Régional Île-de-France",
            "Ville de Nantes", "Métropole de Montpellier", "Mairie de Strasbourg",
            "Préfecture de Loire-Atlantique", "Ville de Rennes", "Communauté d'Agglomération de Nice"
        ]
        ...
        for i in range(limit):
            ...
            days_ago = i * 2
            pub_date = base_date - timedelta(days=days_ago)
            deadline_date = pub_date + timedelta(days=21 + (i % 45))  # 21-66 days deadline
...
async def get_all_available_platforms() -> Dict[str, Any]:
    ...
    return {
        "active_platforms": [
            {"id": "TED", "name": "Tenders Electronic Daily", "country": "EU", "status": "active"},
            {"id": "BOAMP_FR", "name": "BOAMP France", "country": "FR", "status": "active"}
        ],
        "planned_platforms": platforms,
        "total_countries_covered": len(set([p["country"] for p in platforms] + ["EU", "FR"])),
        "integration_roadmap": {
