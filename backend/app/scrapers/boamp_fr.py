...
class BOAMPFRScraper(BaseScraper):
        ...
        try:
            ...
            while len(tenders) < limit:
                ...
                for tender_summary in page_tenders:
                    ...
                    try:
                        detailed_tender = await self._fetch_tender_details(tender_summary)
                        if detailed_tender:
            ...
            self.logger.info(f"Successfully fetched {len(tenders)} tenders from BOAMP France")
            return tenders
        ...
        try:
            ...
            for item in tender_items:
                try:
                    ...
                    date_elem = item.css_first(".date, .publication-date, .date-publication")
    ...
    async def _fetch_tender_details(self, tender_summary: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def _parse_tender_details(self, html_content: str, summary: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            summary_text = self._clean_text(summary_elem.text()) if summary_elem else None
