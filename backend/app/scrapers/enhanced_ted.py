...
class EnhancedTEDScraper(BaseScraper):
    ...
    async def fetch_tenders_by_country(self, country: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if len(tenders) < limit:
                portal_tenders = await self._fetch_eu_data_portal(country, limit - len(tenders))
    ...
    async def _fetch_ted_search_api(self, country: str, limit: int) -> List[Dict[str, Any]]:
            ...
            for rss_url in rss_feeds:
                ...
                try:
                    async with self.session:
                        ...
                        if response.status_code == 200:
                            rss_tenders = self._parse_ted_rss(response.text, country, limit - len(tenders))
                            tenders.extend(rss_tenders)
                except Exception as e:
    ...
    async def _fetch_eu_data_portal(self, country: str, limit: int) -> List[Dict[str, Any]]:
    ...
    def _parse_ted_search_response(self, data: Dict[str, Any], country: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for item in results:
                tender = {
                    "tender_ref": item.get("reference", f"TED-{country}-{datetime.now().year}{len(tenders):06d}"),
                    "source": f"TED_{country}",
                    "title": item.get("title", f"{country} Procurement Notice"),
                    "summary": item.get("summary", f"{country} public procurement from TED API"),
    ...
    def _parse_ted_rss(self, rss_content: str, country: str, limit: int) -> List[Dict[str, Any]]:
        """Parse TED RSS feed."""
        tenders = []
        ...
        try:
            ...
            for i, item in enumerate(items):
                ...
                if country.upper() not in title_text.upper() and country.upper() not in desc_text.upper():
                    continue
                ...
                tender = {
                    ...
                    "publication_date": self._parse_rss_date(pub_date.text if pub_date else None),
    ...
    def _parse_eu_portal_response(self, data: Dict[str, Any], country: str) -> List[Dict[str, Any]]:
                ...
                for i, resource in enumerate(resources):
                    if resource.get("format", "").upper() in ["CSV", "JSON", "XML"]:
                        tender = {
                            ...
                            "summary": resource.get("description", f"{country} procurement from EU Data Portal"),
                            "publication_date": self._parse_date(resource.get("created")),
                            ...
                            "currency": "EUR",
                            "url": resource.get("url", f"https://data.europa.eu/resource/{resource.get('id', i)}")
                        }
                        tenders.append(tender)
...
async def fetch_enhanced_ted_tenders(country: str, limit: int = 50) -> List[Dict[str, Any]]:
...
async def fetch_all_enhanced_ted_tenders(countries: List[str], limit_per_country: int = 20) -> List[Dict[str, Any]]:
    ...
    tasks = [
        fetch_enhanced_ted_tenders(country, limit_per_country)
